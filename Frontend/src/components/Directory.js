...
import { OFFICES, AIRLINES } from "@/components/constdata";
    ...
    const filteredOffices = useMemo(() => {
        return OFFICES.filter((office) => {
            ...
            const matchesAirline =
                !selectedAirline || office.airlineId === selectedAirline;
            const matchesCountry =
                !selectedCountry || office.country === selectedCountry;

            return matchesSearch && matchesAirline && matchesCountry;
        });
    }, [search, selectedAirline, selectedCountry]);

    const countries = useMemo(() => {
        const set = new Set(OFFICES.map((o) => o.country));
                                        ...
                                        >
                                            <div className="flex items-center space-x-4 flex-1">
                                                <div className="h-16 w-16 rounded-xl overflow-hidden bg-gray-100 flex-shrink-0">
                                                    <SafeImage
                                                        src={office.image}
                                                        alt={`${office.airlineName} ${office.city}`}
